<template>
    <div class="messages">
        <v-card elevation="5" class="main chat-box">
          <v-container>
            <div class="first-box commonText--text" v-if="!selectedUser.id">
              <div class="border">Choose a chat...</div>
            </div>
            <div
            ...
            >
              <div class="chat-header d-flex flex-row">
                  <div class="d-sm-none d-block px-2 py-6"><v-icon @click="selectUser({})">mdi-arrow-left</v-icon></div>
                ...
                <div class="px-5 py-6">
                  <span class="fs-large">{{ selectedUser.name }}</span>
                </div>
                <v-spacer></v-spacer>
              </div>
              <div>
                <div id="chat-body" class="chat-body">
                    <div v-if="selectedChat.messages.length">
                        <div v-for="(msg, index) in selectedChat.messages" :key="index">
                            ...
                            <v-col :class="msg.self ? 'text-right' : 'text-left'">
                                ...
                                >
                                <div
                                    class="d-flex flex-row justify-space-between px-1"
                                >
                                ...
                                <div class="text-right pt-1">
                                    <span class="fs-small">{{ msg.time }}</span>
                                    <span v-if="msg.self" class="pl-3">
                    ...
                    <div class="text-center" v-else>say somthing to {{selectedUser.name}}!</div>
                </div>                  
              </div>
